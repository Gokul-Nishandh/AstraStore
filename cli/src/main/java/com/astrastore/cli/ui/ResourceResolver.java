...
public final class ResourceResolver {
    ...
    /**
     * Resolve an object reference like "bucket-name/file.txt" or UUID to a (bucketId, objectKey).
    ...
    public static ResolvedObject resolveObject(String ref, AstraHttpClient client) {
        ...
        return new ResolvedObject(bucket.uuid, bucket.id, key, bucket.name + "/" + key);
    }
    ...
    private static ResolvedObject resolveObjectByUuid(String uuidStr, AstraHttpClient client) {
        try {
            ...
            return new ResolvedObject(
                    UUID.fromString((String) data.get("bucketId")),
                    (String) data.get("bucketId"),
                    (String) data.get("key"),
                    (String) data.get("key"));
        } catch (Exception e) {
            return null;
    ...
    public static List<ResolvedBucket> listAllBuckets(AstraHttpClient client) {
        try {
            List<Map<String, Object>> data = client.get("/api/v1/buckets?size=200",
    ...
    public record ResolvedObject(UUID bucketUuid, String bucketId, String key, String displayName) {
    }
}
